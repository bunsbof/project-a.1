...
@onready var pussy = $".."
@onready var right_spread_col = $RightSpreadCol
@onready var hand_or_tongue = $"../../HandOrTongue"
@onready var timer = $Timer

var cursor_in_spread = false
var dragging = false
var backward_timer : Timer

#func _ready():
	#backward_timer = Timer.new()
	#backward_timer.connect("timeout", Callable(self, "_on_backward_timer_timeout"))

func update_frame(cursor_position):
	var shape = right_spread_col.shape
	var width = 0
	var total_frames = 6
	if shape is RectangleShape2D:
		var rectangle_shape = shape
		width = rectangle_shape.extents.x * 2
	var mouse_frame = int(round((cursor_position / width) * (total_frames - 1))) - 2
